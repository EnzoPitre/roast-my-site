...
export async function generateMetadata({ params }: { params: { lang: string } }): Promise<import('next').Metadata> {
  ...
  const canonical = `${BASE_URL}/${lang}/changelog`;
  return {
    title: t['meta.changelog.title'],
    description: t['meta.changelog.description'],
    keywords: lang === 'fr'
      ? ['changelog roast my site', 'mises à jour audit site web', 'nouveautés roast my site']
      : ['changelog', 'roast my site updates', 'website audit tool releases'],
    alternates: { canonical, languages: { en: `${BASE_URL}/en/changelog`, fr: `${BASE_URL}/fr/changelog` } },
    openGraph: { title: t['meta.changelog.title'], description: t['meta.changelog.description'], url: canonical, type: 'website' },
  };
}
